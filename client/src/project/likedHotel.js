    ...
    <div className="" id="container-package1">
      ...
      (data.length!==0)?(
        ...
        {data.map((item)=>(
            ...
            <img class="card-img-top" src={`/Image/${item.photos}`} alt="Card image cap"/>
            <div class="card-body">
              <h5 class="card-title">{item.city}</h5>
              <div style={{display:"flex",justifyContent:"space-between",fontWeight:"500"}}><a class="btn btn-success" id={item.city} name={item.country} onClick={event=>handleClick(event)}>View Now</a>
